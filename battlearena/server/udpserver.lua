...
local function timesync(session, localtime, from)
	...
	--snax.printf("sync after localtime %s  session %d ", localtime, session)

end

local function udpdispatch(str, from)
	local localtime, eventtime, session = string.unpack("<III", str, 9)
	--snax.printf("recv localtime %s eventtime %d session %d ", localtime, eventtime, session)
	local s = S[session]
	if s then
		...
		-- 	snax.printf("drop this package")
		-- 	-- drop this package, and force time sync
		-- 	return timesync(session, localtime, from)
		-- elseif s.lastevent and eventtime < s.lastevent then
		-- 	snax.printf("drop older event")
		-- 	-- drop older event
		-- 	return
		-- end
		-- s.lastevent = eventtime
	--	snax.printf("send localtime %s eventtime %d session %d ", localtime, eventtime, session)
