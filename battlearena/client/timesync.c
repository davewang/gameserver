...
float _deltaTime ;
int _nextDeltaTimeZero ;
struct timeval *_lastUpdate ;
static int
lcalculateDeltaTime(lua_State *L)
{
    struct timeval now;
    ...
    if (gettimeofday(&now, NULL) != 0)
    {
       // printf("error in gettimeofday");
        _deltaTime = 0;
	    lua_pushnumber(L, _deltaTime);
        return 1;
    }

    // new delta time. Re-fixed issue #1277
    if (_nextDeltaTimeZero == 1 )
    {
        _deltaTime = 0;
        _nextDeltaTimeZero = 0;
    }
    else
    {
...
static int 
lcurrentMillSecond(lua_State *L)
{
    long lLastTime = 0;
...
luaopen_timesync(lua_State *L) {
	...
	luaL_Reg l[] = {
		...
		{ "currentMillSecond", lcurrentMillSecond },
		{ "calculateDeltaTime", lcalculateDeltaTime },
		{ "sleep", lsleep },
		{ NULL, NULL },
	};
    ...
    _lastUpdate = malloc(sizeof( struct timeval));
	return 1;
}
