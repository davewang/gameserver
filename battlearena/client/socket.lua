...
local function tcp_sync(self)
	...
	local data = string.pack("<I", self.__session)
	data = crypt.hmac_hash(self.__secret, data) .. data
	data = string.pack("<I2",string.len(data)+2)..data
	--print(string.format("sync data len %d is %s",string.len(data),data))
	--self.__fd:send(data)
...
local function tcp_send(self, data)
	data = string.pack("<I", self.__session) .. data
	data = crypt.hmac_hash(self.__secret, data) .. data
	data = string.pack("<I2",string.len(data)+2)..data
	--print(string.format("send data len %d is %s",string.len(data),data))
	--self.__fd:send(data)
	self.post(self,data)
end
...
local function tcp_recv(self)
	...
	if string.len(str) >= 2 then
		...
		if string.len(str) >= sz then
			...
			--return str:sub(offset, offset+(sz-2))
			return session,data:sub(5)
		end
		--self.__read = str:sub(nl+1)
		--return str:sub(1, nl-1)
	end
   -- print("lsocket.select begen")
...
--	while lsocket.select({self.__fd}) do
--		str = str .. self.__fd:recv()
--		if string.len(str) >= 2 then
--		    local sz,offset = string.unpack("<I2", str)
--			if string.len(str) >= sz then
...
----		end
--		data = data:sub(5)
--		print(string.format( "rec data %s",data))
--		if data ~= "" then
